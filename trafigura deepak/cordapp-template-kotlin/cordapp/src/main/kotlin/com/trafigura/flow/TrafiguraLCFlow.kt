...
import com.trafigura.dao.ApplicationDao
...
object TrafiguraLCFlow {

    @InitiatingFlow
    @StartableByRPC
    class CreateORUpdateLC(private val lcState: LCState, private val createNewLC: Boolean, private val listOfParticipants:MutableList<Party>) : FlowLogic<SignedTransaction>() {
        ...
        override fun call(): SignedTransaction {
            ...
            applicationDao.createOrUpdateLCOnTargetPeers(lcState, serviceHub.myInfo.legalIdentities.first(), listOfParticipants, createNewLC)
    ...
    @InitiatedBy(CreateORUpdateLC::class)
...
@StartableByRPC
class fetchLCDetailsList() : FlowLogic<String>() {
...
@StartableByRPC
class checkUserExists(val userDetails: UserDetails) : FlowLogic<UserDetails>() {
    override fun call(): UserDetails {
        val applicationDao = ApplicationDao()
        return applicationDao.checkUserExists(userDetails)
    }
}

@StartableByRPC
class createLCAsDraft(val lcState: LCState) : FlowLogic<Unit>() {
    override fun call() {
        val applicationDao = ApplicationDao()
        applicationDao.createLCAsDraft(lcState, serviceHub.myInfo.legalIdentities.first())
        /*val qryCriteriaUnconsumed = QueryCriteria.VaultQueryCriteria(Vault.StateStatus.UNCONSUMED)
        val lcStateref = serviceHub.vaultService.queryBy<LCState>(qryCriteriaUnconsumed).states.single()
        return  lcStateref.state.data.lcDetails.toString();*/
    }
